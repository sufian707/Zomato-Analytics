...
select a.userid ,sum(b.price) as total_amt_spnt from sales a inner join product b on a.product_id=b.product_id
group by a.userid ;

/*2) for how many days has each customer visited zomato*/
select userid, count(distinct created_date) distint_days from sales group by userid;

/* 3) first product purchased by user*/
select * from
(select *,rank() over(partition by userid order by created_date) rnk from sales) a where rnk =1;

/*4)  what is the most purchased item on the menu and how many times it was purchased by all customers  */
select product_id,count(product_id) as cnt from sales group by product_id order by cnt desc limit 1;
